### 
## Prelim attempts at feature work with Rossman Kaggle data
...
dat.store = read_csv("../RossmanKaggleData/store.csv")
...
dat %>% mutate(weekday = lubridate::wday(Date) ) %>%
  group_by(weekday) %>%
  summarise(mean(Sales))

t.lm <- dat %>%
  mutate(weekday = as.factor(lubridate::wday(Date) )) %>%
  inner_join(., dat.store, by="Store")  %>%
  filter(StoreType=="b") %>%
  lm(Sales~weekday, data =.)


## what happens if you try to cluster on dat.store?
## a little tricky with the categorical data
## do the lame thing with a cart tree to predict sales first
dat <- dat %>% inner_join(., 
                          select(dat.store, Store, StoreType, Assortment, CompetitionDistance), by="Store")

fit <- rpart(Sales~StoreType + Assortment + CompetitionDistance, 
             method="anova", control=rpart.control(minsplit = 2, cp=0.01) , 
            data=dat )
...
ggplot(aes(CompetitionDistance, Sales)) + 
  geom_point() + facet_grid(Assortment~StoreType)



##Alternative clustering based on demand estimation and a tree?
