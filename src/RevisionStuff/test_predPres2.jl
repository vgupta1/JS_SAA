###
# Pred/Pres Synthetic Experiment 2
##
#### True distributions drawn as dirichlet
#Anchors depend on features X (features drawn gaussian) + a noise term (MNL)
...
h_grid = range(.1, stop=10, length=100)

Random.seed!(seed)

#####
## Generating True distributions
Xs = X_std * randn(d, K) #Features are Gaussian
...
#setup output
f = open("temp2Results_$(alpha0)_$(sigma)_$(numRuns)_$(seed).tsv", "w")
