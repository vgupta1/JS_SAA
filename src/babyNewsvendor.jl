...
nv_sim_path(ps, Ns) = @.sum(rand(Bernoulli(ps), Ns))
...
function nv_oracle_both(mhats, ps, p0_grid, alpha_grid, Nhats, ss)
...
function nv_loo_both(mhats, p0_grid, alpha_grid, Nhats, ss)
	out = zeros(length(alpha_grid))
	loo_perf = Inf
	p0LOO = 0.
	alphaLOO = 0.
